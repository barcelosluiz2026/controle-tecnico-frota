...
def pane_detail(id):
    ...
    for step in steps:
        html += f"<p>🛠 {step.description}<br><small>{step.created_at.replace(tzinfo=ZoneInfo('')).astimezone(ZoneInfo('America/Sao_Paulo')).strftime('%d/%m/%Y %H:%M')} - {step.created_by}</small></p>"
    ...
    for p in pendencias:
        html += f"<p>📦 {p.tipo_item} - {p.descricao}<br><small>{p.created_at.replace(tzinfo=ZoneInfo('')).astimezone(ZoneInfo("America/Sao_Paulo")).strftime('%d/%m/%Y %H:%M')} - {p.created_by}</small></p>"
